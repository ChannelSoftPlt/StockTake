...
public class ImportFileUpdateDialog extends DialogFragment implements View.OnClickListener {
    View rootView;
    ...
    public void objectSetting(){
        ...
        if (mArgs != null) {
            ...
            exportFileUpdateDialogEditTextCategory.setText(category_name);
        }
        ...
        switch (view.getId()){
            ...
            case R.id.fragment_category_update_dialog_button_ok:
                if(!exportFileUpdateDialogEditTextCategory.getText().toString().equals("")){
                    updateDialogCallBack.updateCategoryItem(exportFileUpdateDialogEditTextCategory.getText().toString(), category_id);
                    closeKeyBoard();
                    dismiss();
                }
                else{
